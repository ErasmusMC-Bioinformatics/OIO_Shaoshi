...
cell_annotation = 'Hepatocytes'
prediction = list(prediction['Hepatocyte'])

preset = pd.read_csv('%s.TestTarget.txt'%cell_annotation,index_col=0,delimiter='\t')
preset = list(preset['Hepatocytes'])
...
plt.title('Hepatocyte (GSE115469 - GSE146409)',fontsize=15)
plt.text(0.2,0.8,'r = %s'%(round(pearsonr(preset,prediction)[0],4)))
ticks = [0,0.2,0.4,0.6,0.8,1]
plt.xticks(ticks)
plt.yticks(ticks)
plt.tight_layout()
plt.savefig('GSE115469.GSE146409.%s.pdf'%cell_annotation,format='pdf')
